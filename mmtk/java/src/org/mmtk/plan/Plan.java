...
//  public static final int LOS_SIZE_THRESHOLD = SegregatedFreeListSpace.MAX_CELL_SIZE;
 ...
 // public static final boolean REQUIRES_LOS = org.mmtk.vm.VM.activePlan.constraints().requiresLOS();
  public static final int MAX_NON_LOS_DEFAULT_ALLOC_BYTES = org.mmtk.vm.VM.activePlan.constraints().maxNonLOSDefaultAllocBytes();
  public static final int MAX_NON_LOS_NONMOVING_ALLOC_BYTES = org.mmtk.vm.VM.activePlan.constraints().maxNonLOSNonMovingAllocBytes();
  public static final int MAX_NON_LOS_COPY_BYTES = org.mmtk.vm.VM.activePlan.constraints().maxNonLOSCopyBytes();


/* Do we support a log bit in the object header?  Some write barriers may use it */
  public static final boolean NEEDS_LOG_BIT_IN_HEADER = org.mmtk.vm.VM.activePlan.constraints().needsLogBitInHeader();
  ...
  public static final Space vmSpace = org.mmtk.vm.VM.memory.getVMSpace();
  ...
  public void postBoot() {
    org.mmtk.vm.VM.statistics.perfEventInit(Options.perfEvents.getValue());
    if (Options.verbose.getValue() > 2) Space.printVMMap();
    if (Options.verbose.getValue() > 3) org.mmtk.vm.VM.config.printConfig();
  ...
  public void replacePhase(int oldScheduledPhase, int scheduledPhase) {
    org.mmtk.vm.VM.assertions.fail("replacePhase not implemented for this plan");
  }
  ...
  public static void setGCStatus(int s) {
	...
	Stats.startGC();
        org.mmtk.vm.VM.activePlan.global().printPreStats();
      }
    }
    org.mmtk.vm.VM.memory.isync();
    gcStatus = s;
    org.mmtk.vm.VM.memory.sync();
    if (gcStatus == NOT_IN_GC) {
      /* From any phase to NOT_IN_GC */
      if (Stats.gatheringStats()) {
        Stats.endGC();
        org.mmtk.vm.VM.activePlan.global().printPostStats();
      }
    }
  }
  ...
  public static Extent usedMemory() {
    return Conversions.pagesToBytes(org.mmtk.vm.VM.activePlan.global().getPagesUsed());
  }
  ...
  public static Extent reservedMemory() {
    return Conversions.pagesToBytes(org.mmtk.vm.VM.activePlan.global().getPagesReserved());
  }
  ...
  public final boolean poll(boolean spaceFull, Space space) {
    if (isCollectionTriggered()) {
      ...
      org.mmtk.vm.VM.collection.joinCollection();
      return true;
    }
    ...
    if (collectionRequired(spaceFull)) {
      ...
      org.mmtk.vm.VM.collection.triggerCollection(Collection.RESOURCE_GC_TRIGGER);
      return true;
    }

    if (concurrentCollectionRequired()) {
      logPoll(space, "Triggering collection");
      org.mmtk.vm.VM.collection.triggerCollection(Collection.INTERNAL_PHASE_GC_TRIGGER);
      return true;
    }
  ...
  public static void checkForAsyncCollection() {
    if (awaitingAsyncCollection && org.mmtk.vm.VM.collection.noThreadsInGC()) {
      awaitingAsyncCollection = false;
      org.mmtk.vm.VM.collection.triggerAsyncCollection(Collection.RESOURCE_GC_TRIGGER);
    }
  }
  ...
  public void startGCspyServer(int port, boolean wait) {
    org.mmtk.vm.VM.assertions.fail("startGCspyServer called on non GCspy plan");
  }
  ...
  public boolean willNeverMove(ObjectReference object) {
    if (!org.mmtk.vm.VM.activePlan.constraints().movesObjects())
      return true;
