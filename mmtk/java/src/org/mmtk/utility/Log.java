...
public class Log implements Constants {
  ...
  public static void write(char[] c, int len) {
    if (org.mmtk.vm.VM.VERIFY_ASSERTIONS) org.mmtk.vm.VM.assertions._assert(len <= c.length);
    for (int i = 0; i < len; i++) {
      add(c[i]);
    }
  }
  ...
  private static Log getLog() {
    if (org.mmtk.vm.VM.assertions.runningVM()) {
      return org.mmtk.vm.VM.activePlan.log();
    } else {
      return log;
    }
  }
  ...
  private void addToBuffer(String s) {
    if (bufferIndex < MESSAGE_BUFFER_SIZE) {
      bufferIndex += org.mmtk.vm.VM.strings.copyStringToChars(s, buffer, bufferIndex, MESSAGE_BUFFER_SIZE + 1);
  ...
  private void flushBuffer() {
    ...
    if (threadIdFlag) {
      org.mmtk.vm.VM.strings.writeThreadId(buffer, totalMessageSize);
    } else {
      org.mmtk.vm.VM.strings.write(buffer, totalMessageSize);
    }
    threadIdFlag = false;
    overflow = false;
    overflowLastChar = '\0';
